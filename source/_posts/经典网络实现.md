...
GAN一直面临着G,D训练困难、G,D的损失函数与训练好坏无关等问题，在此基础上便提出了WGAN，相对于传统的GAN,WGAN只做了几点改动确有很好的效果
...
所以WGAN的损失函数改为了
$$
E_{z\in p_z(z)}[-logD(G(z))] = KL(P_g||P_{data})-2JS(P_{data}||P_g) 
$$
...
数据集kaggle上随便找~~啊，kaggle真香，各方意义上~~，龙书里面的提到的数据集在这里https://pan.baidu.com/s/1Yn53uxFLCbja13_6Ay44MA数据集来源在[这里](https://zhuanlan.zhihu.com/p/24767059),我开始没看issue没找到这个数据集，爬到一半才看到😓
